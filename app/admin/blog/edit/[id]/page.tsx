...
export default async function EditBlogPostPage({ params, searchParams }: PageProps) {
    ...
    const supabase = process.env.SUPABASE_SERVICE_KEY ? createAdminClient() : await createServerSupabase();
    ...
    const defaultStatus: "draft" | "scheduled" | "published" =
    ...
    const scheduledDefault = post.scheduled_for
        ...
        : "";
    const tagsDefault = Array.isArray(post.tags) ? post.tags.join(", ") : "";
    ...
    const updatePost = async (formData: FormData) => {
        ...
        if (!title) {
            redirect(`/admin/blog/edit/${postId}?error=Title%20is%20required`);
        }
        if (!content) {
            redirect(`/admin/blog/edit/${postId}?error=Content%20cannot%20be%20empty`);
        }
        ...
        if (statusInput === "scheduled" && scheduledDate && isNaN(scheduledDate.getTime())) {
            redirect(`/admin/blog/edit/${postId}?error=Invalid%20schedule%20date`);
        }
        ...
        const publishedAt = isPublished ? (post.published_at || nowIso) : null;

        let imageUrl = imageUrlInput || post.image_url || "";
        if (!imageUrl && coverFile && coverFile.size > 0) {
            if (!process.env.SUPABASE_SERVICE_KEY) {
                redirect(`/admin/blog/edit/${postId}?error=Image%20upload%20needs%20SUPABASE_SERVICE_KEY%20and%20blog-assets%20bucket`);
            }
            ...
            if (upload.error) {
                redirect(`/admin/blog/edit/${postId}?error=${encodeURIComponent(`Image upload failed: ${upload.error.message}`)}`);
            }
        ...
        if (updateError) {
            redirect(`/admin/blog/edit/${postId}?error=${encodeURIComponent(updateError.message)}`);
        }
    ...
    const errorMsg = paramsObj?.error;
            ...
            <form action={updatePost} className="space-y-10">
                <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="tags" name="tags" defaultValue={tagsDefault} />
                        <p className="text-xs text-gray-500">Comma-separated; used for internal linking.</p>
                    </div>
                </section>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            id="canonical_url"
                            name="canonical_url"
                            defaultValue={post.canonical_url || `${SITE_URL}/blog/${post.slug}`}
                        />
                    </div>
                    <div className="space-y-2">
                ...
                <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="scheduled_for" name="scheduled_for" type="datetime-local" defaultValue={scheduledDefault} />
